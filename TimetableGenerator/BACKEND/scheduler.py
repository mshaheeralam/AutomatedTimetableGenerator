...
from BACKEND.utils import load_data, show_timetable, set_up, show_statistics, write_solution_to_file
from BACKEND.costs import check_hard_constraints, hard_constraints_cost, empty_space_groups_cost, \
    empty_space_teachers_cost, \
    free_hour
import copy
import math
from BACKEND.excel import core
...
def main():
    ...
    subjects_order: dictionary where key = (name of the subject, index of the group), value = [int, int]
    where ints represent start times (row in matrix) for types of classes P and L respectively
    ...
    data = load_data(
        'C:\\Users\\shahe\\OneDrive\\Desktop\\TimetableGenerator\\TimetableGenerator\\BACKEND\\test_files\\' + file,
